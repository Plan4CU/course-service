...
public class CourseController {
    ...
    public ResponseEntity<PagedModel<EntityModel<Course>>> getAllCourses(@PageableDefault(size = 20) Pageable pageable) {
        ...
        List<EntityModel<Course>> courses = coursePage.getContent().stream()
                .map(course -> EntityModel.of(course,
                        ...
                        linkTo(methodOn(SectionController.class).getSectionsByCourseId(course.getCourseId(), Pageable.unpaged())).withRel("sections")
                ))
                .collect(Collectors.toList());
        Link selfLink = linkTo(methodOn(CourseController.class).getAllCourses(pageable)).withSelfRel();
        PagedModel.PageMetadata pageMetadata = new PagedModel.PageMetadata(
                coursePage.getSize(), coursePage.getNumber(), coursePage.getTotalElements(), coursePage.getTotalPages());
        ...
        if (coursePage.hasNext()) {
            pagedModel.add(linkTo(methodOn(CourseController.class)
                    .getAllCourses(pageable.next())).withRel("next"));
        }
        if (coursePage.hasPrevious()) {
            pagedModel.add(linkTo(methodOn(CourseController.class)
                    .getAllCourses(pageable.previousOrFirst())).withRel("prev"));
        }
    ...
    public ResponseEntity<EntityModel<Course>> getCourseById(@PathVariable String id) {
        ...
        EntityModel<Course> entityModel = EntityModel.of(course,
                ...
                linkTo(methodOn(SectionController.class).getSectionsByCourseId(id, Pageable.unpaged())).withRel("sections"),
                linkTo(methodOn(CourseController.class).getAllCourses(Pageable.unpaged())).withRel("courses"));
    ...
    public ResponseEntity<EntityModel<Course>> createCourse(@RequestBody Course course) {
        ...
        EntityModel<Course> entityModel = EntityModel.of(newCourse,linkTo(methodOn(CourseController.class).getCourseById(newCourse.getCourseId())).withSelfRel(),
                linkTo(methodOn(CourseController.class).getAllCourses(Pageable.unpaged())).withRel("courses"));
        return ResponseEntity.created(linkTo(methodOn(CourseController.class).getCourseById(newCourse.getCourseId())).toUri())
                .body(entityModel);
    }
    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('course_service:write')")
