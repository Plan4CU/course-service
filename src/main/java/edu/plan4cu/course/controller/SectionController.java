...
public class SectionController {
    ...
    public ResponseEntity<PagedModel<EntityModel<Section>>> getSectionsByCourseId(
        ...
        Link selfLink = linkTo(
                methodOn(SectionController.class).getSectionsByCourseId(courseId, pageable)).withSelfRel();
        PagedModel.PageMetadata pageMetadata = new PagedModel.PageMetadata(
                sectionPage.getSize(), sectionPage.getNumber(), sectionPage.getTotalElements(),
        ...
        if (sectionPage.hasNext()) {
            pagedModel.add(
                    linkTo(methodOn(SectionController.class).getSectionsByCourseId(courseId, pageable.next())).withRel(
                            "next"));
        }
    ...
    public ResponseEntity<EntityModel<Section>> getSectionById(@PathVariable int id) {
        ...
        EntityModel<Section> entityModel = EntityModel.of(section,
                ...
                linkTo(methodOn(CourseController.class).getCourseById(section.getCourse().getCourseId())).withRel(
                        "course"));
    ...
    public ResponseEntity<EntityModel<Section>> createSection(@RequestBody Section section) {
        ...
        EntityModel<Section> entityModel = EntityModel.of(newSection,
                ...
                linkTo(methodOn(CourseController.class).getCourseById(newSection.getCourse().getCourseId())).withRel(
                        "course"));
        return ResponseEntity.created(
                        linkTo(methodOn(SectionController.class).getSectionById(newSection.getSectionId())).toUri())
                .body(entityModel);
    }
    ...
    public ResponseEntity<EntityModel<Section>> updateSection(@PathVariable int id, @RequestBody Section section) {
        ...
        EntityModel<Section> entityModel = EntityModel.of(updatedSection,
                linkTo(methodOn(SectionController.class).getSectionById(id)).withSelfRel(),
                linkTo(methodOn(CourseController.class).getCourseById(
                        updatedSection.getCourse().getCourseId())).withRel("course"));
        return ResponseEntity.ok(entityModel);
    }
